...
values plot as an \(h \times w\) RGB image, and \(f_{CV}(.)\) maps the
image to an estimated distance \(\hat{D} \in [0, +\infty)\). Details of
...
To train the computer vision model for computing \(\hat{D}\) (as
...
annotate reliably \citep{nikolenko2021synthetic}. This approach is
...
Accordingly, we generated a synthetic dataset consisting of 80,000
training and 8,000 test residual plots, providing full control over the
data-generating process, precise computation of \(D\), cost-effective
scaling, and coverage of a wide range of visual patterns associated with
model violations.
...
error (RMSE) across 100 trials. The tuning process included early
