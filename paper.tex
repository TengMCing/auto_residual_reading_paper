...
null plots. While the true plot's distance \(D\) depends on the
data-generating process, null plots have \(D = 0\). We used our
optimized computer vision model to estimate \(D\) for all plots. To
ensure fair comparison, we reject \(H_0\) if the true plot has the
highest estimated distance. Conventional tests, including the Ramsey
...
\(\hat{D}\) on true plots. While the performance is slightly worse than
on the test data provided in Table \ref{tab:performance}, the mean
absolute error remains low and the correlation between predictions and
true values remains high. Consistent with results in Table
...
the true plot and all null plots, and \(\bar{d}_{\text{null}}^{(j)}\) is
the mean distance from null plot \(j\) to other nulls. These averages
help address asymmetries in pairwise distances.
...
when it is positive. A negative \(\delta_{\text{adj}}\) suggests some
null plots may appear more visually distinctive to humans than the true
plot. In some instances, the weighted detection rate is close to one,
yet the \(\delta_{\text{adj}}\) is negative. This discrepancy implies
