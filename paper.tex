...
accomplished through the lineup protocol (Figure \ref{fig:ex-lineup}),
where the true residual plot is embedded within a field of null plots.
If the true residual plot can be distinguished from the lineup, it
provides evidence for rejecting \(H_0\).
...
an \(h \times w\) RGB image, \(\boldsymbol{s}\) is a vector of auxiliary
features including scagnostics measures and sample size, and
\(f_{CV}(\cdot)\) maps the image to an estimated distance
\(\hat{D} \in [0, +\infty)\). Details of the model architecture and
training procedure are provided in Section \ref{sec-model-architecture}
and \ref{sec-model-data-generation}, respectively.
...
formula \citep{series2011studio}, as data points are plotted in black on
a white background.
