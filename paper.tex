...
\(Q\), the fallback is to evaluate Equation \ref{eq:kl-1} numerically.
...
In order to approximate \(D_{KL}(P \| Q)\) in a practically computable
...
\(D_{KL}(P \| Q)\) can be approximated by
...
\noindent where \(\hat{D}_{KL}^{(i)}(P \| Q)\) is the estimator of the
...
To estimate this distance from a residual plot, we proposed a computer
...
quantiles can be found in \citet{hyndman1996sample}. For instance, if
...
To enable supervised training of the computer vision models for
detecting violations in linear regression, we require paired residual
plots and their corresponding \(D\) values to train a regression model.
We therefore generated a synthetic dataset consisting of 80,000 training
and 8,000 test residual plots. This simulation-based approach provides
full control over the data-generating process, enabling precise
computation of \(D\), cost-effective scaling of the dataset, and the
ability to model a wide range of visual patterns associated with model
violations.
...
representing a distinct range of \(D\). Table \ref{tab:data-overall}
...
\begin{tabular}[t]{lrr}
\toprule
Violations & \#train samples & \#test samples\\
\midrule
no violations & 1546 & 155\\
non-linearity & 22184 & 2218\\
heteroskedasticity & 10826 & 1067\\
non-linearity + heteroskedasticity & 10221 & 985\\
non-normality & 10797 & 1111\\
non-linearity + non-normality & 8835 & 928\\
heteroskedasticity + non-normality & 7870 & 819\\
non-linearity + heteroskedasticity + non-normality & 7721 & 717\\
\bottomrule
\end{tabular}
\end{table}

The computer vision model was trained on paired data consisting of an
RGB residual plot image as input and the corresponding target value
\(D\), with the objective of learning to estimate \(\hat{D}\). Model
...
\caption{\label{tab:performance-sub}The train and test performance of the $32 \times 32$ model presented with different model violations.}
\centering
\begin{tabular}[t]{lrr}
\toprule
Violations & Train RMSE & Test RMSE\\
\midrule
no violations & 1.149 & 1.267\\
non-linearity & 0.625 & 0.787\\
heteroskedasticity & 0.528 & 0.602\\
non-linearity + heteroskedasticity & 0.593 & 0.751\\
non-normality & 0.279 & 0.320\\
non-linearity + non-normality & 0.472 & 0.600\\
heteroskedasticity + non-normality & 0.354 & 0.489\\
non-linearity + heteroskedasticity + non-normality & 0.432 & 0.620\\
\bottomrule
\end{tabular}
\end{table}
