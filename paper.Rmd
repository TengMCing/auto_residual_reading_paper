...
This corresponds to the standard lineup protocol, where the observed plot must be identified as the most visually distinct plot.

More generally, the empirical distribution of $\{\hat{D}_{null}^{(i)}\}_{i=1}^{m}$ provides an estimate of the null distribution of the test statistic. Let $Q_{null}(\alpha)$ denote the empirical $\alpha$-quantile of this distribution. A level-$\alpha$ test is then given by $$\text{Reject } H_0 \quad \Longleftrightarrow \quad \hat{D} \ge Q_{null}(\alpha).$$

Equivalently, a Monte Carlo $p$-value can be computed as
$$\hat{p} = \frac{1}{m + 1} + \frac{1}{m + 1}\sum_{i=1}^{m} I\left( \hat{D}_{null}^{(i)} \ge \hat{D} \right).$$
