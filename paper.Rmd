...
\end{equation*}
\noindent where $V_{h \times w}(\cdot)$ renders a residuals vs. fitted values plot as an $h \times w$ RGB image, and $f_{CV}(\cdot)$ maps the image to an estimated distance $\hat{D} \in [0, +\infty)$. Details of the model architecture and training procedure are provided in Section \ref{sec-model-architecture} and \ref{sec-model-data-generation}, respectively.
