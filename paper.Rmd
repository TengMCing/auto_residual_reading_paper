...
\noindent where $V_{h \times w}(.)$ renders the residuals vs. fitted values plot as an $h \times w$ RGB image, and $f_{CV}(.)$ is a model that maps the image to an estimated distance $\hat{D} \in [0, +\infty)$. TODO if this citation is appropraite to demonstrate that synthetic data generation to train neural network is common \citep{fanSelectiveOverviewDeep2021}. 
