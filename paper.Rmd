...
For null plots ($D = 0$), over-prediction was expected (see Section \ref{sec-model-lineup-evaluation}), but not under-prediction. Therefore, we analysed the relationship between residuals and all the factors involved in the data generating process. We found that most deviations stemmed from non-linearity and the presence of a second predictor in the simulation model, where small $\varepsilon$ variances caused under-predictions and large $\varepsilon$ variances caused over-prediction, as illustrated in Figure \ref{fig:over-under}. 

To investigate further, we stratified the test set by violation type and re-evaluated performance (Table \ref{tab:performance-sub}). It was found that null plots had the poorest metrics, while non-normality cases were easiest to predict, with RMSE around $0.3$. Non-linearity was harder to assess than heteroskedasticity or non-normality. Plots with multiple violations showed intermediate performance.
