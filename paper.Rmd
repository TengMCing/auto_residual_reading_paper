...
This measure was introduced in @li2024plot to quantify the effect size of non-linearity and heteroskedasticity in residual plots. In the presence of model misspecification, if the error structure deviates from $\boldsymbol{e} \sim N(\boldsymbol{0}_n,\sigma^2\boldsymbol{I}_n)$, for instance with covariance $\boldsymbol{V}$ and omitted higher-order terms $\boldsymbol{Z}\boldsymbol{\beta}_z$, the induced residual distribution can be expressed as $P = N(\boldsymbol{R}\boldsymbol{Z}\boldsymbol{\beta}_z, \text{diag}(\boldsymbol{R}\boldsymbol{V}\boldsymbol{R}))$, where the diagonal approximation ensures non-degenerate covariance matrix. The transformation $D = \log(1 + D_{KL})$ compresses large values of $D_{KL}$, improves numerical stability during estimation (Section \ref{sec-model-distance-estimation}), and ensures that $D$ is well-defined when $D_{KL} = 0$.
...
<!-- We define a distance measure Equation \ref{eq:kl-0} between the true residual distribution $P$ and a reference distribution $Q$, which is only directly computable when the data-generating process is known. In practice, this is unavailable, as residual diagnostics are typically motivated by its unknown structure. -->

<!-- To approximate this distance from a residual plot, we introduce a computer vision estimator -->

<!-- $$\hat{D} = f_{CV}\big(V_{h \times w}(\hat{\boldsymbol{e}}, \hat{\boldsymbol{y}})\big),$$ -->

<!-- where $V_{h \times w}(\cdot)$ converts the residual–fitted values plot into an $h \times w$ RGB image, and $f_{CV}(\cdot)$ maps the image to an estimated distance $\hat{D} \in [0,\infty)$. -->

<!-- The estimator $\hat{D}$ serves as an empirical index of deviation from the ideal residual pattern, but it does not recover the true distance $D$ exactly. This is because a single residual plot is only a stochastic summary of $P$, and can vary noticeably across realizations, particularly for small $n$, inducing variability in $\hat{D}$ depending on how representative the plotted realization is. $\hat{D}$ by its own is also not very reliable in making decision for detecting model   -->

<!-- To use $\hat{D}$ statistically, Section \ref{sec-model-statistical-testing} -->
