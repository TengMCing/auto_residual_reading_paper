...
# To install Python and the required libraries, you may use the following script
if (TRUE) {
  # Install `miniconda`
...
To support supervised training of the computer vision models for detecting violations in linear regression, we require paired residual plots and their corresponding $D$ values to train a regression model. In deep learning, training computer vision models with synthetic data is a common practice, particularly when labelled data are scarce, costly to obtain, or difficult to annotate reliably [@nikolenko2021synthetic]. This approach is especially suitable when the underlying data-generating process is well understood and large numbers of controlled examples can be produced, so the model can learn visual patterns associated with specific violations without the cost and ambiguity of manual labelling. We therefore generated a synthetic dataset consisting of 80,000 training and 8,000 test residual plots. This simulation-based approach provides full control over the data-generating process, ensures precise computation of $D$, supports cost-effective scaling, and captures a wide range of visual patterns associated with model violations.
