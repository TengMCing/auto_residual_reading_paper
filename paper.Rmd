...
When the error vector $\boldsymbol{e}$ is non-Gaussian, the induced residual distribution $P$ is generally non-Gaussian and typically not available in closed form. In this case, the Gaussian closed-form expression in Equation \ref{eq:kl-2} is no longer applicable, and the KL divergence must be evaluated from its definition in Equation \ref{eq:kl-1}.

However, since the residual vector $\hat{\boldsymbol{e}} = \boldsymbol{R}\boldsymbol{e}$ is a linear transformation of $\boldsymbol{e}$, the joint density $p(\hat{\boldsymbol{e}})$ is generally intractable even when the marginal distribution of $\boldsymbol{e}$ is known. Moreover, direct numerical evaluation of Equation \ref{eq:kl-1} involves an $n$-dimensional integral over $\mathbb{R}^n$, which becomes computationally infeasible for large $n$.

To obtain a tractable approximation, we impose a working assumption that the residuals $\{\hat{e}_i\}_{i=1}^n$ are mutually independent, i.e., $p(\hat{\boldsymbol{e}}) \approx \prod_{i=1}^n p_i(e_i)$. Under this factorisation, the KL divergence decomposes additively:
...
where $P_i$ denotes the marginal distribution of $\hat{e}_i$ with density $p_i$, and $Q_i$ denotes the corresponding marginal under $Q$ with density $q_i$. The independence assumption generally does not hold, but it avoids high-dimensional integration and relies only on univariate densities, thereby substantially reducing computational cost.
