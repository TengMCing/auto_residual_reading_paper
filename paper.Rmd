...
the residuals are given by the difference between the observed and fitted values. Under a correctly specified model, the residuals can be expressed as a linear transformation of the error term,
...
The reference distribution $Q$ assumes a correctly specified model. Under model misspecification, the true residual distribution $P$ differs from $Q$. For example, omitted variables correlated with columns of $\boldsymbol{X}$ alter the residual structure and therefore change $P$. Likewise, if the error term follows a non-normal distribution, such as a multivariate lognormal distribution, the residual distribution will typically exhibit skewness and heavier tails.


## Distance of $P$ from $Q$ {#sec-distance-of-p-from-q}

We define the distance of the true residual distribution $P$ from the reference residual distribution $Q$ using the Kullback–Leibler divergence [@kullback1951information]:
