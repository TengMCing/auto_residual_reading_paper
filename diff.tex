...
%DIF DEL paper-submission1.tex   Mon Jan 12 20:09:37 2026
%DIF ADD paper.tex               Thu Feb 19 15:09:44 2026
% v1.04 - May 2023
...
\DIFdelbegin \DIFdel{heteroscedasticity}\DIFdelend \DIFaddbegin \DIFadd{heteroskedasticity}\DIFaddend , and non-normality. The presence of structure in
...
accomplished through the lineup protocol \DIFaddbegin \DIFadd{(Figure \ref{fig:ex-lineup})}\DIFaddend ,
where the true residual plot is embedded within a \DIFdelbegin \DIFdel{lineup alongside several }\DIFdelend \DIFaddbegin \DIFadd{field of }\DIFaddend null plots.
...
\DIFaddend The practice of delivering a residual plot as a lineup is generally
...
human \DIFdelbegin \DIFdel{judgements }\DIFdelend \DIFaddbegin \DIFadd{judgments }\DIFaddend \citep[see][]{li2024plot}. Unlike conventional
...
the residual \DIFdelbegin \DIFdel{plots }\DIFdelend \DIFaddbegin \DIFadd{plot }\DIFaddend diagnostics workflow, addressing the need for
\DIFdelbegin \DIFdel{an
}\DIFdelend automated visual inference. The \DIFaddbegin \DIFadd{model is a regression-based computer
...
quantifying the extent of model violations. The model performance is
evaluated on held-out test plots and compared with classical residual
diagnostic tests as well as results from a human-subject experiment.
}

\DIFadd{The }\DIFaddend paper is structured as follows. Section
...
get the empirical estimates. For \DIFdelbegin \DIFdel{simplicity, in }\DIFdelend \DIFaddbegin \DIFadd{the purpose of }\DIFaddend this study, we replaced
...
\(N(\boldsymbol{0}_n, \text{diag}(\boldsymbol{R}\sigma^2))\). \DIFaddbegin \DIFadd{This
replacement ensures a non-degenerate distribution, which is required for
computing the distance defined in Section
\ref{sec-distance-of-p-from-q}.
}\DIFaddend 
...
\(Q\)}{Distance of P from Q}}\DIFdelbegin %DIFDELCMD < \label{distance-of-p-from-q}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{sec-distance-of-p-from-q}
\DIFaddend 
...
D = \log\left(1 + D_{KL}\DIFaddbegin \DIFadd{(P \| Q)}\DIFaddend \right),
\end{equation}

where \DIFdelbegin \DIFdel{\(D_{KL}\) }\DIFdelend \DIFaddbegin \DIFadd{\(D_{KL}(P \| Q)\) }\DIFaddend is defined as

\begin{equation} \label{eq:kl-1}
D_{KL}\DIFaddbegin \DIFadd{(P \| Q) }\DIFaddend = \int_{\mathbb{R}^{n}}\log\frac{p(\boldsymbol{e})}{q(\boldsymbol{e})}p(\boldsymbol{e})d\boldsymbol{e},
\end{equation}
...
semi-definite matrix, \DIFdelbegin \DIFdel{\(Q\) }\DIFdelend \DIFaddbegin \DIFadd{\(P\) }\DIFaddend can be represented as
...
D_{KL}\DIFaddbegin \DIFadd{(P \| Q) }\DIFaddend = \frac{1}{2}\left(\log\frac{|\boldsymbol{W}|}{|\text{diag}(\boldsymbol{R}\sigma^2)|} - n + \text{tr}(\boldsymbol{W}^{-1}\text{diag}(\boldsymbol{R}\sigma^2)) + \boldsymbol{\mu}_z^\top\boldsymbol{W}^{-1}\boldsymbol{\mu}_z\right),
\end{equation}
...
\(Q\), the fallback is to \DIFdelbegin \DIFdel{solve }\DIFdelend \DIFaddbegin \DIFadd{evaluate }\DIFaddend Equation \ref{eq:kl-1} numerically.
...
In order to approximate \DIFdelbegin \DIFdel{\(D_{KL}\) }\DIFdelend \DIFaddbegin \DIFadd{\(D_{KL}(P \| Q)\) }\DIFaddend in a practically computable
...
Kullback-Leibler divergence evaluated for each individual residual due
to the assumption of independence between observations. Second, it is
...
for any \(1 \leq i < j \leq n\), but its existence is essential for
...
\DIFdelbegin \DIFdel{\(D_{KL}\) }\DIFdelend \DIFaddbegin \DIFadd{\(D_{KL}(P \| Q)\) }\DIFaddend can be approximated by

\begin{align*} \label{eq:kl-3}
D_{KL}\DIFaddbegin \DIFadd{(P \| Q) }\DIFaddend &\approx \sum_{i = 1}^{n} \hat{D}_{KL}^{(i)}\DIFaddbegin \DIFadd{(P \| Q)}\DIFaddend , \quad
\hat{D}_{KL}^{(i)}\DIFaddbegin \DIFadd{(P \| Q) }\DIFaddend = \frac{1}{m}\sum_{j = 1}^{m} \log\frac{\hat{p}_i(B_{ij})}{q(B_{ij})},
\end{align*}

\noindent where \DIFdelbegin \DIFdel{\(\hat{D}_{KL}^{(i)}\) }\DIFdelend \DIFaddbegin \DIFadd{\(\hat{D}_{KL}^{(i)}(P \| Q)\) }\DIFaddend is the estimator of the
...
To \DIFdelbegin \DIFdel{approximate }\DIFdelend \DIFaddbegin \DIFadd{estimate }\DIFaddend this distance from a residual plot, we proposed a computer
vision estimator \(\hat{D}\) formulated as
...
values plot as an \(h \times w\) RGB image, and \(f_{CV}(.)\) \DIFdelbegin \DIFdel{is a model
that }\DIFdelend maps the
image to an estimated distance \(\hat{D} \in [0, +\infty)\). \DIFaddbegin \DIFadd{Details of
the model architecture are provided in Section
\ref{sec-model-architecture}, and the training procedure is described in
Section \ref{sec-model-data-generation}.
}\DIFaddend 
...
maximum estimated distance among the \DIFdelbegin \DIFdel{\(m - 1\) }\DIFdelend \DIFaddbegin \DIFadd{19 }\DIFaddend null plots, denoted as
\DIFdelbegin \DIFdel{\(\max\limits_{1 \leq i \leq m-1} {\hat{D}_{null}^{(i)}}\)}\DIFdelend \DIFaddbegin \DIFadd{\(\max\limits_{1 \leq i \leq 19} {\hat{D}_{null}^{(i)}}\)}\DIFaddend , where
...
\DIFdelbegin \DIFdel{\({\hat{D}_{null}^{(i)}}\) }\DIFdelend \DIFaddbegin \DIFadd{\(\hat{D}_{null}^{(i)}\) }\DIFaddend can be viewed as an approximation \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{to }\DIFaddend the null
...
for decision-making purposes. \DIFdelbegin \DIFdel{The details of the sample quantile
computation }\DIFdelend \DIFaddbegin \DIFadd{This follows the standard simulation-based
...
quantiles }\DIFaddend can be found in \citet{hyndman1996sample}. For instance, if
...
estimated distance for null plots \DIFdelbegin \DIFdel{with a }\DIFdelend \DIFaddbegin \DIFadd{at the }\DIFaddend 95\% significance level. \DIFdelbegin \DIFdel{Based
on our experience}\DIFdelend \DIFaddbegin \DIFadd{In
practice}\DIFaddend , at least 100 null plots are typically \DIFdelbegin \DIFdel{needed for stable
estimation, but }\DIFdelend \DIFaddbegin \DIFadd{required for stable
estimate of the null distribution, though }\DIFaddend more may be \DIFdelbegin \DIFdel{required if the
null distribution has
}\DIFdelend \DIFaddbegin \DIFadd{necessary if the
distribution exhibits }\DIFaddend heavy tails. Alternatively, a \(p\)-value \DIFdelbegin \DIFdel{is the probability}\DIFdelend \DIFaddbegin \DIFadd{can be
defined as the probability, under \(H_0\), }\DIFaddend of observing a distance \DIFdelbegin \DIFdel{equally or greater than }\DIFdelend \DIFaddbegin \DIFadd{at
least as large as }\DIFaddend \(\hat{D}\)\DIFdelbegin \DIFdel{under \(H_0\)}\DIFdelend , and it can be estimated as
...
{\centering \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=1\linewidth]{paper_files/figure-latex/cnn-diag-1}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=1\linewidth]{figures/cnn} 
\DIFaddendFL 
...
To \DIFdelbegin \DIFdel{enable supervised training of computer vision models for detecting
violations in linear regression, we }\DIFdelend \DIFaddbegin \DIFadd{train the computer vision model for computing \(\hat{D}\) (as
described in Section \ref{sec-model-distance-estimation}), we used
paired residual plots and their corresponding \(D\) values. In deep
learning, it is common to train computer vision models with synthetic
data, especially when labelled data are scarce, costly, or difficult to
annotate reliably \mbox{%DIFAUXCMD
\citep{nikolenko2021synthetic}}\hskip0pt%DIFAUXCMD
. This approach is
particularly suitable when the underlying data-generating process is
well understood and large numbers of controlled examples can be
produced, allowing the model to learn visual patterns associated with
specific violations without the cost and ambiguity of manual labelling.
Accordingly, we }\DIFaddend generated a synthetic dataset \DIFaddbegin \DIFadd{consisting }\DIFaddend of 80,000
training and 8,000 test residual plots\DIFdelbegin \DIFdel{. This simulation-based
approach provided }\DIFdelend \DIFaddbegin \DIFadd{, providing }\DIFaddend full control over the
data-generating process, \DIFdelbegin \DIFdel{allowing }\DIFdelend precise computation of \(D\), cost-effective
\DIFdelbegin \DIFdel{data }\DIFdelend scaling, and \DIFdelbegin \DIFdel{the ability to model diverse visual patterns of }\DIFdelend \DIFaddbegin \DIFadd{coverage of a wide range of visual patterns associated with
}\DIFaddend model violations.
...
\DIFadd{The computer vision model was trained on paired data consisting of an
RGB residual plot image as input and the corresponding target value
\(D\), with the objective of learning to estimate \(\hat{D}\). }\DIFaddend Model
...
\citep{omalley2019kerastuner}, minimizing validation \DIFdelbegin \DIFdel{RMSE }\DIFdelend \DIFaddbegin \DIFadd{root mean square
error (RMSE) }\DIFaddend across 100 trials. The tuning process included early
stopping and considered dropout rate, batch normalization, input
resolution, and auxiliary inputs.
...
To investigate further, we stratified the \DIFaddbegin \DIFadd{train and }\DIFaddend test set by
violation type and re-evaluated performance (Table
\DIFdelbegin \DIFdel{\ref{tab:performance-sub}}\DIFdelend \DIFaddbegin \DIFadd{\ref{tab:data-overall}}\DIFaddend ). It was found that null plots had the poorest
...
\caption{\label{tab:performance}The test performance of three optimized models with different input sizes. \DIFaddbeginFL \DIFaddFL{The metrics are computed by comparing the estimated distance $\hat{D}$ (model output) and the target distance $D$ generated from the synthetic data model. RMSE represents the root mean squared error; $R^2$ is the squared correlation between the two quantities; MAE denotes the mean absolute error; and Huber loss refers to the average Huber loss computed over the test set.}\DIFaddendFL }
\centering
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
%DIFDELCMD < \label{tab:performance-sub}%%%
\DIFdelFL{The test performance of the $32 \times 32$ model presented with different model violations.}}
%DIFAUXCMD
%DIFDELCMD < \centering
%DIFDELCMD < \begin{tabular}[t]{lrr}
%DIFDELCMD < \toprule
%DIFDELCMD < %%%
\DIFdelFL{Violations }%DIFDELCMD < & %%%
\DIFdelFL{\#samples }%DIFDELCMD < & %%%
\DIFdelFL{RMSE}%DIFDELCMD < \\
%DIFDELCMD < \midrule
%DIFDELCMD < %%%
\DIFdelFL{no violations }%DIFDELCMD < & %%%
\DIFdelFL{155 }%DIFDELCMD < & %%%
\DIFdelFL{1.267}%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{non-linearity }%DIFDELCMD < & %%%
\DIFdelFL{2218 }%DIFDELCMD < & %%%
\DIFdelFL{0.787}%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{heteroskedasticity }%DIFDELCMD < & %%%
\DIFdelFL{1067 }%DIFDELCMD < & %%%
\DIFdelFL{0.602}%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{non-linearity + heteroskedasticity }%DIFDELCMD < & %%%
\DIFdelFL{985 }%DIFDELCMD < & %%%
\DIFdelFL{0.751}%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{non-normality }%DIFDELCMD < & %%%
\DIFdelFL{1111 }%DIFDELCMD < & %%%
\DIFdelFL{0.320}%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{non-linearity + non-normality }%DIFDELCMD < & %%%
\DIFdelFL{928 }%DIFDELCMD < & %%%
\DIFdelFL{0.600}%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{heteroskedasticity + non-normality }%DIFDELCMD < & %%%
\DIFdelFL{819 }%DIFDELCMD < & %%%
\DIFdelFL{0.489}%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{non-linearity + heteroskedasticity + non-normality }%DIFDELCMD < & %%%
\DIFdelFL{717 }%DIFDELCMD < & %%%
\DIFdelFL{0.620}%DIFDELCMD < \\
...
%DIFDELCMD < %%%
\DIFdelend \subsection{Comparison with Human Visual Inference and Conventional
...
\DIFdelbegin \DIFdel{\ref{tab:performance-sub}}\DIFdelend \DIFaddbegin \DIFadd{\ref{tab:data-overall}}\DIFaddend , non-linearity cases are harder to predict than
...
respect to the \DIFdelbegin \DIFdel{greyscale }\DIFdelend \DIFaddbegin \DIFadd{grayscale }\DIFaddend input, shows that the top-right and
...
(\DIFdelbegin \DIFdel{\(p-\text{value} = 0.00498\)}\DIFdelend \DIFaddbegin \DIFadd{\(p\text{-value} = 0.00498\)}\DIFaddend ), leading to rejection of \(H_0\). The
...
\citep{kableextra}, \texttt{patchwork} \citep{patchwork}, \DIFdelbegin \texttt{\DIFdel{rcartocolor}} %DIFAUXCMD
\DIFdel{\mbox{%DIFAUXCMD
\citep{rcartocolor}}\hskip0pt%DIFAUXCMD
, }\DIFdelend \texttt{glue}
\citep{glue}, \texttt{ggpcp} \citep{ggpcp}, \texttt{here} \citep{here},
\DIFdelbegin \texttt{\DIFdel{magick}} %DIFAUXCMD
\DIFdel{\mbox{%DIFAUXCMD
\citep{magick}}\hskip0pt%DIFAUXCMD
, }\DIFdelend and \texttt{reticulate} \citep{reticulate}.
...
contains all materials required to reproduce this article\DIFaddbegin \DIFadd{.
}

\DIFadd{We acknowledge the use of ChatGPT (OpenAI, GPT-4, accessed July 2025) to
...
\DIFadd{The data that support the findings of this study are openly available in
the project's GitHub repository
(}\url{https://github.com/TengMCing/auto_residual_reading_paper}\DIFadd{)}\DIFaddend .
